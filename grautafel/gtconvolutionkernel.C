...
const struct ConvolutionKernel GT::sobelX = {3, 3, // xsiz, ysiz
        1, 1, // xorig, yorig
        (unsigned char *) sobelXMatrix, // data
        1, 8, // nNum, nDen
};

const struct ConvolutionKernel GT::sobelY = {3, 3, // xsiz, ysiz
        1, 1, // xorig, yorig
        (unsigned char *) sobelYMatrix, // data
        1, 8  // nNum, nDen
};

const struct ConvolutionKernel GT::prewittX = {3, 3, //xsiz, ysiz
        1, 1, //xorig, yorig
        (unsigned char *) prewittXMatrix, //data
        1, 6 // nNum, nDen
};
const struct ConvolutionKernel GT::prewittY = {3, 3, //xsiz, ysiz
        1, 1, //xorig, yorig
        (unsigned char *) prewittYMatrix, //data
        1, 6 //nNum, nDen
};

const struct ConvolutionKernel  * const GT::edgeDetectorX = &sobelX, * const GT::edgeDetectorY = &sobelY;
...
int ConvolutionKernel::convolveSquaredNotNormalised(const QImage *img, QPoint point) const {
...
int ConvolutionKernel::convolveSquared(const QImage *img, QPoint point, int additionalMultiplier) const {
  return sqrt(convolveSquaredNotNormalised(img, point) * sq(additionalMultiplier) / sq(normDenominator) / 9);
}

double ConvolutionKernel::dConvolveSquared(const QImage *img, QPoint point) const {
